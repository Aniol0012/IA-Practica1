...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        currentPosition, visited_old = state
        successors = []
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                visited = tuple(cv or cp == next_pos for cp, cv in zip(self.corners, visited_old))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    # If there are no corners left not visited, then all corners have been visited
    if len(corners_not_visited) == 0:
        return 0

    # Finds the distance from current position to the nearest corner
    distances = []
